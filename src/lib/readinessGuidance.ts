...
function fundamentalsMissingCount(
...
function schemaMissingCount(
  contract: DataContract,
  validation: ValidationResult,
): number {
  const fieldCount = contract.dataset.reduce(
...
export function computeSectionGuidance(
  ...
  const fundamentals: SectionGuidanceInfo =
    fundMissing > 0
      ? {
          status: "incomplete",
  ...
  const schema: SectionGuidanceInfo =
    schemaMissing > 0
      ? {
          status: "incomplete",
  ...
  const stakeholders: SectionGuidanceInfo =
    contactCount > 0
      ? {
          status: "complete",
          missingCount: 0,
...
export function buildReadinessGuidanceItems(
  ...
  const schemaOk =
    !validation.errors.some((e) => e.section === "schema") && fieldCount > 0;
  const contactCount = countAssignedStakeholders(contract.stakeholders);
...
export function issueToFieldId(
  issue: ValidationIssue,
): ReadinessFieldId | undefined {
