...
export function validateContract(
  ...
  if (!id.trim()) {
    ...
    issues.push({
      code: "id-derived",
      message:
        "Contract ID must match the name (auto-derived slug with stable suffix).",
      severity: "error",
      section: "fundamentals",
      fieldId: "contract-id",
    });
  ...
  for (const table of dataset) {
    ...
    for (const col of table.columns) {
      ...
      if (
        col.classification &&
        !isValidClassificationValue(col.classification)
      ) {
        issues.push({
    ...
    for (const rel of table.relationships ?? []) {
      if (!isExportedRelationshipType(rel.type)) {
        ...
        isLegacySingleColumnBelongsTo(rel) &&
        isLegacyBelongsToIncomplete(rel)
      ) {
        issues.push({
        ...
        (rel.type === "composite_foreign_key" ||
          isCompositeTableRelationship(rel)) &&
  ...
  for (const row of contract.slaProperties ?? []) {
    ...
    if (!row.property) {
      issues.push({
        ...
        message:
          "Each SLA row needs a property type (for example latency or retention).",
        severity: "error",
        section: "sla",
      });
    ...
    if (row.unit?.trim() && !isValidSlaUnit(row.unit)) {
      issues.push({
        ...
        message:
          "SLA unit must be one of: d, day, days, y, yr, years, h, hr, hours.",
        severity: "error",
        section: "sla",
      });
    }
    ...
    if (row.element?.trim() && !isValidSlaElement(row.element)) {
      issues.push({
        ...
        message:
          "SLA element must use Object.Property notation (comma-separated for multiple).",
        severity: "error",
        section: "sla",
      });
    }
  ...
  const piiCount = dataset.reduce(
    (a, t) => a + t.columns.filter((c) => c.isPII).length,
    0,
  );
  ...
  const publishStatusOk = canPublishFromStatus(
    info.status,
    contract.inRevision,
  );
