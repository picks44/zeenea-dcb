...
export function mapQualityRulesToYaml(
  rules: QualityRule[],
  ...
  return ensureQualityRuleIds(rules)
    ...
    .map((q) => {
      ...
      if (q.businessImpact?.trim())
        rule.businessImpact = q.businessImpact.trim();
      stripUndefined(rule);
      return rule;
    });
}

export function mapTagsToYaml(
...
export function mapPropertyItemsToYaml(
  items: PropertyItems,
): Record<string, unknown> {
  ...
  if (items.logicalType === "object" && items.properties?.length) {
    out.properties = items.properties.map((p) => {
      ...
      if (isValidOdcsLogicalType(p.logicalType))
        nested.logicalType = p.logicalType;
...
export function isAuthoritativeDefinitionEmpty(
  d: AuthoritativeDefinition,
): boolean {
  return !d.url?.trim() && !d.type?.trim() && !d.description?.trim();
}

export function isAuthoritativeDefinitionComplete(
  d: AuthoritativeDefinition,
): boolean {
  return !!d.url?.trim() && !!d.type?.trim();
}

export function isAuthoritativeDefinitionPartial(
  d: AuthoritativeDefinition,
): boolean {
  return (
    !isAuthoritativeDefinitionEmpty(d) && !isAuthoritativeDefinitionComplete(d)
  );
}

export function getAuthoritativeLinkFieldErrors(def: AuthoritativeDefinition): {
  url?: string;
  type?: string;
} {
  if (
    isAuthoritativeDefinitionEmpty(def) ||
    isAuthoritativeDefinitionComplete(def)
  ) {
    return {};
  }
...
export function filterCustomPropertiesForSave(
  props: CustomProperty[],
): CustomProperty[] {
  return props.filter(
    (p) => p.property?.trim() || p.value?.trim() || p.description?.trim(),
  );
}

export function migrateExamplesField(
