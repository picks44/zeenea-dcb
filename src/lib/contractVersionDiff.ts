...
import { compareGovernanceSnapshots, type GovernanceSnapshotDiff } from './governanceSnapshotDiff'
...
export function compareContractVersions(
  ...
  const hasExportChange = !exportDocumentsEqualIgnoringPublishFields(left, right)
...
export function summarizeChangesSince(
  ...
  return compareContractVersions(previous, contractToComparisonSnapshot(current))
}

export function buildWorkingCopySummaryLines(comparison: ContractVersionComparison): string[] {
...
export function buildPublishChangelog(comparison: ContractVersionComparison): string {
  if (!comparison.hasAnyChange) return ''

  const lines: string[] = []

  if (comparison.hasExportChange) {
    const exportLines = buildPublishChangelogFromExportDiff(comparison.exportDiff)
...
function syntheticExportChangelogLines(diff: ExportedContractDiff): string[] {
  ...
  if (schema.added || schema.removed || schema.updated || diff.schemaTables.changed) {
    lines.push('Updated schema')
  }
  if (customProperties.added || customProperties.removed || customProperties.updated) {
  ...
  if (diff.summaryLines.some(l => l.includes('reference') || l.includes('Reference'))) {
    lines.push('Updated reference links')
  }
  if (diff.summaryLines.some(l => l.includes('relationship'))) {
    lines.push('Updated relationships')
  }
  if (diff.summaryLines.some(l => l.includes('description') || l.includes('metadata'))) {
