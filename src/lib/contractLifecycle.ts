...
export function applyLifecycleAction(
  ...
  switch (action) {
    ...
    case "publish":
      return canTransitionStatus(status, "active") || status === "draft"
        ? "active"
        : status;
    case "deprecate":
...
export function canPublishFromStatus(
  status: LifecycleStatus,
  inRevision?: boolean,
): boolean {
...
export function isContractEditableStatus(
  status: LifecycleStatus,
  inRevision?: boolean,
): boolean {
...
export function isImportSectionEditable(
  status: LifecycleStatus,
  isViewer: boolean,
): boolean {
