...
export function findZeeneaCatalogItemByUrl(url: string): ZeeneaCatalogItem | undefined {
  const trimmed = url.trim()
  return ZEENEA_CATALOG.find(item => item.url === trimmed)
}

export function findZeeneaCatalogItemById(id: string): ZeeneaCatalogItem | undefined {
