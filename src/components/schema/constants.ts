...
export const LOGICAL_TYPES: TypeConfig[] = [
  ...
  { value: 'time',      pmLabel: 'Time',         techLabel: 'time',      hint: '14:30:00',                 icon: Clock,       color: 'bg-orange-50 text-orange-700 border-orange-100',  iconBg: 'bg-orange-50',  iconColor: 'text-orange-700' },
...
/** Keep ODCS name in sync when the user edits physicalName (UI primary field). */
export function syncColumnNameFromPhysical(
  physicalName: string,
  currentName?: string,
): string {
  const trimmed = physicalName.trim()
  if (!trimmed) return currentName?.trim() || ''
  if (!currentName?.trim()) return normalizeOdcsName(trimmed)
  return normalizeOdcsName(trimmed)
}
